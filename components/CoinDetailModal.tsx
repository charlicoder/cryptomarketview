...
import { CryptoCurrency, useGetCoinChartQuery } from '@/store/services/cryptoApi'
...
export default function CoinDetailModal({ coin, isOpen, onClose }: CoinDetailModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`max-w-5xl max-h-[90vh] overflow-y-auto p-0 ${isDark
          ? 'bg-slate-800 border-slate-700 text-white'
          : 'bg-white border-gray-200 text-gray-900'
        }`}>
        {/* Header */}
        <div className={`flex items-center p-6 border-b ${isDark ? 'border-slate-700' : 'border-gray-200'
          }`}>
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <DialogDescription className={`text-xs uppercase font-medium ${isDark ? 'text-slate-400' : 'text-gray-500'
                }`}>
                {coin.symbol}
              </DialogDescription>
            </div>
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className={`rounded-lg p-4 border ${isDark
              ...
              <p className={`text-2xl font-bold ${coin.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'
                }`}>
          ...
          <div>
            ...
            <div className={`h-80 rounded-lg border ${isDark
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className={`rounded-lg p-4 text-center border ${isDark
                ? 'bg-slate-700/50 border-slate-600'
                : 'bg-gray-50 border-gray-200'
              }`}>
              <p className={`text-sm mb-2 ${isDark ? 'text-slate-400' : 'text-gray-500'
          ...
          <div className="grid grid-cols-3 gap-4">
            <Button className={`font-medium py-3 ${isDark
